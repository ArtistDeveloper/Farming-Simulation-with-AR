...
public class UtilsClass
{
    ...
    // World에 Text를 생성한다.
    public static TextMesh CreateWorldText(string text, Quaternion parent, Vector3 localPosition = default(Vector3), int fontSize = 40, Color? color = null
    ...
    // World에 Text를 생성한다.
    public static TextMesh CreateWorldText(Quaternion parent, string text, Vector3 localPosition, int fontSize, Color color
    , TextAnchor textAnchor, TextAlignment textAlignment, int sortingOrder)
    {
        ...
        // transform.localEulerAngles = new Vector3(90, 0, 0); //숫자를 눕히려고 임의로 추가한 코드 //원본
        transform.localEulerAngles = parent.eulerAngles;
